...
#  This is the "inert" version, for inert shurikens. It will just print drives
#  and then reboot
...
main(){
  ...
  echo "		+++Ninja Boot'n'Nuke(Inert)+++"
  echo "Inert Device: No wiping will be done."
  ...
  for DEVICE in ${REGDEVS} ;do
      ( fake_wipe ${DEVICE} disk ) &
      PIDS="$PIDS $!"
  done

  for DEVICE in ${P1DEVS};do
      ( fake_wipe ${DEVICE} flash ) &
      PIDS="$PIDS $!"
  done
